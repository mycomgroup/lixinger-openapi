...
### 理杏仁 API（付费）
- **接口数量**: 162 个接口
- **访问方式**: 需要有效的 API Token
- **认证方式**: Token 鉴权（从 `token.cfg` 文件读取）
- **费用**: 付费订阅服务，不同套餐有不同的调用次数限制
- **免费额度**: 部分接口可能有试用额度，具体查看理杏仁官网定价

### AkShare（免费）
- **接口数量**: 1000+ 接口
- **访问方式**: 开源 Python 库，无需 API Key
- **认证方式**: 大部分接口无需认证，部分接口（如集思录）需要 Cookie
- **费用**: 完全免费
- **数据质量**: 数据来源广泛，更新频率各异

### Massive API（付费）
- **接口数量**: 95 个 REST API + 1945 个 Flat Files 数据集
- **访问方式**: 需要 MASSIVE_API_KEY
- **认证方式**: URL 参数 `?apiKey=YOUR_API_KEY`
- **费用**: 商业 API 服务，需付费订阅
- **覆盖范围**: 美股、加密货币、外汇、期货、期权、指数等全球市场数据
- **数据特点**: 专业级金融数据，实时和历史数据全覆盖
- **位置**: `.claude/plugins/query_data/massive-api-docs/`

**总结**: 当前可用的免费数据源主要是 **AkShare**（1000+ 接口），理杏仁和 Massive 均为付费服务。
...
- 理杏仁场景先看 `api_new/API_KEYWORD_INDEX.md` 和 `api_new/api-docs/`
- AkShare 场景再看 `api_new/akshare_data/`
- 未来 provider 按 `api_new/{provider}_data/` 或等价目录挂载
...
- 或使用 `grep -r "关键字" api_new/api-docs/` 搜索（在 lixinger-data-query 目录下执行）
...
- 当理杏仁 API 无法满足需求时使用
- 使用 `grep -r "关键字" api_new/akshare_data/` 搜索（在 lixinger-data-query 目录下执行）
...
### Massive API 接口（付费，全球市场数据）

**使用说明**：
- Massive API（前身为 Polygon.io）提供专业级全球市场数据
- **需要付费订阅**：所有接口需要 `MASSIVE_API_KEY`
- 覆盖美股、加密货币、外汇、期货、期权、指数等
- 数据质量高，适合专业交易和分析
...
**股票数据（Stocks）**：
- 聚合 OHLC 数据：`/v2/aggs/ticker/{ticker}/range/{multiplier}/{timespan}/{from}/{to}`
- 所有股票代码：`/v3/reference/tickers`
- 股票详情：`/v3/reference/tickers/{ticker}`
- 财务报表：`/vX/reference/financials`（已弃用，将移除）
- 分红数据：`/v3/reference/dividends`
- 拆股数据：`/v3/reference/splits`
- IPO 数据：`/vX/reference/ipos`
- 股票新闻：`/v2/reference/news`

**加密货币（Crypto）**：
- OHLC 数据：`/v2/aggs/ticker/{cryptoTicker}/range/{multiplier}/{timespan}/{from}/{to}`
- 加密货币代码：`/v3/reference/tickers?market=crypto`
- 交易所列表：`/v1/meta/crypto-exchanges`

**外汇（Forex）**：
- OHLC 数据：`/v2/aggs/ticker/{forexTicker}/range/{multiplier}/{timespan}/{from}/{to}`
- 货币转换：`/v1/conversion/{from}/{to}`

**期货（Futures）**：
- 期货合约：`/v3/reference/futures/contracts`
- OHLC 数据：`/v2/aggs/ticker/{futuresTicker}/range/{multiplier}/{timespan}/{from}/{to}`

**期权（Options）**：
- 期权合约：`/v3/reference/options/contracts`
- OHLC 数据：`/v2/aggs/ticker/{optionsTicker}/range/{multiplier}/{timespan}/{from}/{to}`

**指数（Indices）**：
- OHLC 数据：`/v2/aggs/ticker/{indicesTicker}/range/{multiplier}/{timespan}/{from}/{to}`

**经济数据（Economy）**：
- 通胀预期：`/v2/economy/expectations/inflation`
- 通胀数据：`/v2/economy/inflation/{indicator}`
- 劳动力市场：`/v2/economy/labor/{indicator}`
- 国债收益率：`/v2/economy/treasury/{maturity}`

#### Flat Files（1945 个数据集）

**说明**：
- 通过 S3 兼容端点访问的历史数据 CSV 文件
- 需要 Access Key 和 Secret Key
- 适合批量下载大量历史数据

**数据类型**：
- **股票**（us_stocks_sip）：交易日聚合、分钟聚合、报价数据、交易数据（2003 年至今）
- **加密货币**（global_crypto）：日聚合、分钟聚合、交易数据（2010 年至今）
- **外汇**（global_forex）：日聚合、分钟聚合、报价数据（2009 年至今）
- **指数**（us_indices）：日聚合、分钟聚合、指数值（2023 年至今）
- **期权**（us_options_opra）：日聚合、分钟聚合、交易数据、报价数据（2014 年至今）
- **期货**：合约数据、交易和报价数据
...
aws s3 cp s3://flatfiles/us_stocks_sip/trades_v1/2025/01/2025-01-15.csv.gz . --endpoint-url https://files.massive.com

# 使用 Python boto3
import boto3
s3 = boto3.client('s3', endpoint_url='https://files.massive.com')
s3.download_file('flatfiles', 'us_stocks_sip/trades_v1/2025/01/2025-01-15.csv.gz', './data.csv.gz')
```

**数据覆盖范围**：
- **股票交易所**：NYSE、Nasdaq、Cboe、MIAX、MEMX、IEX、LTSE、FINRA、OTC 等 19+ 个交易所
- **历史数据**：
  - 股票：2003 年至今
  - 外汇：2009 年至今
  - 加密货币：2010 年至今
...
```bash
# 查看所有 REST API 文档
ls plugins/query_data/massive-api-docs/docs/rest_*.md

# 查看 Flat Files 文档
ls plugins/query_data/massive-api-docs/docs/flat-files_*.md

# 搜索特定接口
grep -r "aggregates" plugins/query_data/massive-api-docs/docs/
```

**文档位置**：
- REST API 文档：`plugins/query_data/massive-api-docs/docs/rest_*.md`
- Flat Files 文档：`plugins/query_data/massive-api-docs/docs/flat-files_*.md`
- 主文档：`plugins/query_data/massive-api-docs/SKILL.md`
...
### Massive API
- **付费服务**：需要订阅并获取 `MASSIVE_API_KEY`
- **认证方式**：URL 参数 `?apiKey=YOUR_API_KEY` 或环境变量 `MASSIVE_API_KEY`
- **错误码**：
  - `401`: API Key 无效
  - `403`: 权限不足（订阅级别不够）
  - `429`: 请求频率超限
- **数据时区**：
  ...
  - 加密货币：UTC
- **时间戳格式**：Unix 毫秒时间戳
- **REST API 基础 URL**：`https://api.massive.com`
- **Flat Files 端点**：`https://files.massive.com`
- **限制**：不同订阅套餐有不同的调用频率和数据访问限制
- **专业数据**：适合专业交易、回测、量化分析等场景
