...
- 所有母策略统一继承全局 `hard guards`、结果去重规则与最小历史验证门槛，不能各自放宽
...
所有策略统一遵循“4 层分析 + 2 个运行时关口”：

1. **硬筛选层**  
   所有母策略先执行全局 `hard guards`，至少包括：`PE-TTM(扣非) > 0` 或 `PE-TTM > 0`、扣非净利润 > 0、经营活动产生的现金流量净额 > 0、排除 `ST`、默认上市满 3 年、并复核近 20 个交易日成交额 / 换手率是否达标。任一核心条件失守，直接降为观察名单或淘汰，不进入后续策略强化层。
...
5. **结果去重层**  
   先按股票篮子 hash 去重，再按策略家族归并。`quant-factor-screener` 与 `esg-screener` 默认只作为解释 / 验证层，不单独计入独立共识票。只有去重后仍来自不同主家族的信号，才允许描述为“多策略同时命中”。

6. **历史验证层**  
   候选规则在升格为高优先策略前，必须对过去若干历史日期点做最小 `walk-forward` 验证，至少比较未来 `5/20/60` 个交易日相对沪深 `300` 的超额收益、胜率与回撤；未过线只能保留为研究假设，不能升级为高优先策略池。
...
2. **潜在机会清单**  
   明确说明机会来源、属于哪一类机会、为什么当前值得看，并标记 `策略家族`、`主承接策略` 与 `去重后角色`（主结论 / 辅助确认 / 仅观察）。
...
- `quality-value.json`：低估值主线的独立基线
- `dividend-quality.json`：高股东回报主线的独立基线
- `small-cap-quality-growth.json`：小盘成长主线的独立基线
...
- 基线模板只负责收敛样本，不替代后续 `hard guards`、去重与历史验证
...
2. 先用与主线匹配的独立基线模板建候选池
3. 对入围股执行统一 `hard guards`，先排除负 PE / 负扣非 / 负经营现金流、`ST`、过新上市与流动性不足样本
4. 再按策略做强化筛选、异常识别与机会分类
5. 对结果先做篮子去重，再按策略家族归并，避免把同一逻辑误当成多重独立证据
6. 仅对入围股补数，不对全市场做重度拉取
7. 需要升格为高优先策略时，补做最小 `walk-forward` 验证后再输出结论、证据、失效条件与后续跟踪信号
