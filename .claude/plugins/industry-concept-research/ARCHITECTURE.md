...
本插件用于整合行业与概念板块研究能力，解决原先多个分散 skill 难以协同的问题。目标是把“行业表现、轮动判断、产业链验证、政策映射”打通成统一研究流水线。
...
1. 行业板块分析（横截面强弱、资金流、成分股）
2. 行业轮动判断（宏观周期、风格切换、超/低配建议）
3. 产业链图谱（上中下游联动、价格/库存/订单代理）
4. 概念板块分析（主题热度、轮动节奏、拥挤度）
5. 政策敏感度（政策—行业映射、情景推演、触发条件）
6. 行业研报分析（机构观点提炼、分歧识别、验证路径）
...
| industry-report-analyzer | 行业研报样本、机构观点 | 共识-分歧矩阵与验证清单 | 将研报转化为可执行监控 |
...
- **Structure Engine（结构引擎）**：行业板块 + 宏观轮动 + 产业链
