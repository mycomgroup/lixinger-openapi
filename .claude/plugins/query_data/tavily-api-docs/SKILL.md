...
| 端点 | 基础消耗 | 高级选项 |
|------|---------|---------|
| Search | 1 credit | advanced: 2 credits |
| Extract | 1 credit / 5 URLs | advanced: 2 credits / 5 URLs |
| Crawl | 1 credit / 10 pages | with instructions: 2 credits / 10 pages |
| Map | 1 credit / 10 pages | with instructions: 2 credits / 10 pages |
| Research | 按需计费 | 基于复杂度 |
...
1. **选择合适的 search_depth**: 
   - `ultra-fast`: 时间敏感场景
   - `fast`: 平衡速度和质量
   - `basic`: 一般用途
   - `advanced`: 高精度需求

2. **使用 include_answer**: 当需要 AI 生成的答案时启用

3. **合理设置 max_results**: 默认 5，根据需求调整 (最大 20)

4. **使用 Project ID**: 多项目时便于跟踪用量

5. **监控用量**: 定期调用 `/usage` 端点监控配额

6. **处理失败结果**: Extract API 会返回 `failed_results` 数组

7. **使用流式响应**: 长时间运行的 Research 任务使用 `stream=True`
